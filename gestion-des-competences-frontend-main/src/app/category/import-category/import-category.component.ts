...
export class ImportCategoryComponent implements OnInit {
  ...
  constructor(public dialogRef: MatDialogRef<ImportCategoryComponent>, private categoryService: CategoryServiceService) { }
  ...
  onSubmit() {
    if (!this.file)
      ...
      // @ts-ignore
      this.categoryService.postFile(formData, {
        observe: 'response',
        responseType: 'text'
      })
      .subscribe((response: string) => {
        ...
        Toast.fire({
          icon: 'warning',
          title: 'Categories added '+ response,
          position: 'top',
          width: 450,
        })
      })
